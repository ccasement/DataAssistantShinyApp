...
shinyServer(function(session, input, output) {
  ...
  showModal(
    modalDialog(
      ...
      h3(style = "color: red; text-align: center;", "The app will automatically
  ...
  observeEvent(input$copy_var, {
    ...
    # count number of columns of df before making any copies -- will use soon
    ...
    for (j in 1:ncol(selected_data)) {
      for (i in 1:input$num_copies) {
        ...
        cols <- ncol(bag$data_new_df)

        # copy selected variable(s) and place into (new) last column(s)
        bag$data_new_df[, cols + 1] <- selected_data[, j]
    ...
    new_col_indices <- (old_n_cols + 1):ncol(bag$data_new_df)
    ...
    updateSelectInput(session, "select_var", NULL,
      choices = names(bag$data_new_df),
      selected = names(bag$data_new_df)[1:input$num_copies]
    )
  ...
  #################################################
	output$download_dataset <- downloadHandler(
		...
		content = function(file) {
		  ...
		  # only include selected vars if user checks that checkbox
# 		  if (input$select_vars_output) {
#
# 		    # sort vars if user checks that checkbox
# 		    if (input$sort_vars_output) {
# 		      final_data <- bag$data_new_df[, input$final_vars_sorted]
# 		    } else {
# 		      final_data <- bag$data_new_df[, input$final_vars]
# 		    }
