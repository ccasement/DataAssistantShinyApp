...
navbarPage("Data Assistant",
  ...
  navbarMenu("Input Data", icon = icon("upload"),
    ...
    tabPanel("Upload Data Set",
      ...
      column(3,
        ...
        p(style = "color: red; font-weight: bold;", "Upload and store a data
          set."
        ),
  ...
  tabPanel("Download Data", icon = icon("download"),
    ...
    column(4,
      ...
      wellPanel(
        checkboxInput("sort_vars_output", strong("Sort Variable(s)")),

        conditionalPanel(
          condition = "input.sort_vars_output",

          p("Sort the variables from first to last."),
  ...
  tabPanel("Help", icon = icon("question-circle"),
    ...
    column(4,
      ...
      p("The app accepts files with the following (common) extensions when
        uploading data: .csv, .xlsx, .xls, .json, .rda, .mtp, .mat, .txt,
        .sas7bdat, .sav, and .dta. For additional options, click",
        a("here",
          href = "https://www.rdocumentation.org/packages/rio/versions/0.5.26",
          target = "_blank"
        ),
        "and scroll down to 'Supported file formats.'"
      ),
    ...
    column(4,
      ...
      p("When downloading the data set, you must type a name for the file. Then
        select from the following list of file extensions: .csv, .xlsx, .json,
        .mat, .txt, .sas7bdat, .sav, and .dta. Two additional options are
        available as well for specifying missing values and selecting specific
  ...
  tabPanel("About", icon = icon("info-circle"),
    ...
    column(4,
      ...
      h4("Developers"),
      #p("This tool was developed by Chris Casement and Laura McSweeney, both in the
      # Department of Mathematics at Fairfield University."
      # ),
      ...
      HTML("<b>Anyone may use this app without the need to cite its developers
        and without fear of copyright infringement.</b>"
      ),
      linebreaks(2),
